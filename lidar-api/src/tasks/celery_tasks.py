...
def pull_image_with_retry(client: docker.DockerClient, image_name: str, image_tag: str, max_retries: int = 3) -> bool:
    """
    ...
    while retry_count < max_retries:
        try:
            logger.info(f"Pulling image {image_name}:{image_tag}, attempt {retry_count + 1}")
            ...
            if auth_error:
                logger.warning(f"Authentication error while pulling image. Image may be private or registry settings incorrect: {e}")
                ...
                try:
                    ...
                    logger.info(f"Image {image_name}:{image_tag} found locally, proceeding without pulling")
                    return True
...
def process_lidar_data(
    ...
    try:
        ...
        if "denied: denied" in error_message:
            ...
            error_details = "Connection to Docker daemon refused. Check if Docker is running."
        else:
